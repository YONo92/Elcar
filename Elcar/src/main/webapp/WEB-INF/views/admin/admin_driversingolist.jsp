							...
							<div class="card my-4">
								<div
									...
									<div
										...
										<h6 class="text-white text-capitalize ps-3">관리자_드라이버 신고 회원
											관리</h6>
									</div>
								</div>
								<c:choose>
												...
												<div class="table-responsive p-0" style="height: 350px">
													<table class="table align-items-center mb-0"
														...
														text-xxs font-weight-bolder opacity-7 ps-4">category
																...
																<th
																	class="text-uppercase text-secondary 
														text-xxs font-weight-bolder opacity-7 ps-1">content</th>
																<th
																	class="text-uppercase text-secondary 
															...
															<c:forEach var="driversingo" items="${driversingo }">
																<tr>
																	...
																	<td><select id='driverPoint${driversingo.num }'
																		class="driverPoint">
																			<c:choose>
																				<c:when test="${driversingo.status eq 0}">
																					<option value="0" selected>대기중</option>
																					<option value="-10">-10 차감</option>
																					<option value="-20">-20 차감</option>
																					<option value="-30">-30 차감</option>
																				</c:when>
																				<c:when test="${driversingo.status eq 1}">
																					<option value="" selected disabled>-10 차감</option>
																				</c:when>
																				<c:when test="${driversingo.status eq 2}">
																					<option value="" selected disabled>-20 차감</option>
																				</c:when>
																				<c:otherwise>
																					<option value="" selected disabled>-30 차감</option>
																				</c:otherwise>
																			</c:choose>
																	</select></td> 
																</tr>
					...
					[이전]&nbsp;
											...
											<c:forEach var="i" begin="${pageInfo.startPage }"
												...
												<c:choose>
													...
													<c:otherwise>
														<a href="admin_driversingolist?page=${i}&mem_text=${mem_text}">[${i }]</a>
													</c:otherwise>
												</c:choose>
											</c:forEach>
											<c:choose>
